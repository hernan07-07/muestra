...
st.markdown("""
    ...
    /* Input y Selectbox Contrast Fix */
    ...
    /* ARREGLO DE CONTRASTE EN EL DESPLEGABLE (COMBO) */
    ...
    /* Espacios y Mesas */
    hr { margin: 5px 0px !important; border-top: 1px solid #263238 !important; }
    .mesa-header { 
...
if check_password():
    ...
    # FORMULARIO (REORDENADO PARA EVITAR EL "ENTER" TEMPRANO)
    with st.expander("➕ AÑADIR REGISTRO", expanded=True):
        with st.form("alta", clear_on_submit=True):
            r1c1, r1c2 = st.columns([1, 2])
            f_m = r1c1.number_input("MESA", min_value=0, step=1, key=f"f_{st.session_state.focus_key}")
            f_c = r1c2.selectbox("CATEGORÍA", ["MAYOR", "ADOLESCENTE", "MENOR", "BEBÉ"])
            
            # El nombre al final para que el Enter aquí sea el disparador lógico
            f_n = st.text_input("APELLIDO y nombre")
            f_o = st.text_input("OBSERVACIONES (Opcional)")
            
            if st.form_submit_button("📥 GUARDAR E INSERTAR", use_container_width=True):
                if f_n:
